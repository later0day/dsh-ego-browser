...
*/
function castWorkerEnv() {
	const target = process.env.EGO_CAST_CDP_URL;
	if (!target || target.trim() === "") return void 0;
		...
		if (now - lastAttempt > 8e3) {
			...
			try {
				...
				ctx.subprocess.spawn({
					...
					cwd: process.cwd(),
					env: {
						...process.versions.electron ? { ELECTRON_RUN_AS_NODE: process.env.ELECTRON_RUN_AS_NODE ?? "1" } : {},
						...castWorkerEnv()
					},
					stdio: {
						stdin: { data: "" },
...
function initCastServer(ctx, cfg, bridge, ffmpegManager, openAgentWindow$1 = async () => ({
	...
	const disposeWatchStatus = server.register({
		...
		handler: async (_req, resRaw) => {
			...
			const port = await ensureWorker();
			return sendJson(res, 200, {
				...(port === null ? null : await proxyFrom(port, "/api/watch/status")) || {
					ok: false,
					state: "idle",
					reason: "worker not ready"
				},
...
*/
const TRUST_REMOTE_UI_ENV = "DSH_TRUST_REMOTE_UI";
/** Truthy values that turn the switch on. Anything else (incl. unset) = off. */
function switchOn(raw) {
...
*/
function registerRemoteUiTrust(ctx) {
	...
	ctx.on("webserver/index-inject", (...args) => {
		const table = args[0];
		if (!Array.isArray(table)) return;
		if (table.some((r) => typeof r === "object" && r !== null && r.name === "__DSH_TRANSPORT__")) return;
		table.push(row);
	});
	ctx.logger?.info?.(`ego-browser: ${TRUST_REMOTE_UI_ENV} on — injecting __DSH_TRANSPORT__.ownsHost so the public-IP UI uses host settings persistence`);
	return true;
}
...
function apply(ctx, config = {}) {
	...
	ctx.inject?.(["webServer"], (wctx) => {
		...
		try {
			registerRemoteUiTrust(wctx);
		} catch (err) {
			ctx.logger?.warn?.(`ego-browser: remote-ui-trust init failed: ${err?.message ?? err}`);
		}
	});
	ctx.effect?.(() => {
...
//#endregion
export { Config, apply, createActiveSpaceTracker, findChromeBinary, inject, name, resolveEgoEnv, runWithStaleSpaceRetry, shouldReapBrowser };